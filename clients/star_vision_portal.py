"""STAR Vision Portal — realidade aumentada gestual local.

Implementação própria inspirada no conceito público de portal entre as mãos.
OpenCV/MediaPipe são dependências opcionais e só são importadas ao executar este cliente.
"""
...
class FilterEngine:
    """Filtros vetorizados; nada fora da máscara do portal é alterado."""
    ...
    def apply(self, key: str, roi, tick: float):
        method = getattr(self, f"_fx_{key}", None)
        return roi.copy() if method is None else method(roi, tick)
    ...
    def _fx_hologram(self, roi, tick):
        ...
        base[:, :, 1] = np.clip(base[:, :, 1] * 1.15 + 18, 0, 255)
        base[:, :, 2] = np.clip(base[:, :, 2] * 0.35, 0, 255)
        phase = int(tick * 45) % 6
        base[phase::6] = (base[phase::6] * 0.52).astype(np.uint8)
        edges = cv2.Canny(gray, 55, 130)
        glow = cv2.GaussianBlur(edges, (0, 0), 4)
        return cv2.addWeighted(base, 0.82, cv2.cvtColor(glow, cv2.COLOR_GRAY2BGR), 0.55, 0)
    ...
    def _fx_vintage(self, roi, tick):
        ...
        max_dist = max(1.0, math.hypot(w / 2.0, h / 2.0))
        vignette = np.clip(1.0 - 0.48 * dist / max_dist, 0.0, 1.0)
        noise = np.random.default_rng(int(tick * 12)).normal(0.0, 5.0, roi.shape)
    ...
    def _fx_night(self, roi, tick):
        ...
        out = np.zeros_like(roi)
        out[:, :, 1] = np.clip(gray * 1.25 + 12, 0, 255)
        out[:, :, 0] = np.clip(gray * 0.28, 0, 255)
        return out.astype(np.uint8)
    ...
    def _fx_xray(self, roi, tick):
        ...
        out = cv2.applyColorMap(inv, cv2.COLORMAP_BONE)
    ...
    def _fx_cyber(self, roi, tick):
        ...
        levels = (gray // 42) * 42
        out = np.zeros_like(roi)
        out[:, :, 0] = np.clip(levels * 1.15 + 28, 0, 255)
        out[:, :, 1] = np.clip((255 - levels) * 0.30 + 28, 0, 255)
        out[:, :, 2] = np.clip(levels * 0.74 + 65, 0, 255)
...
class PortalRenderer:
    """Máscara suavizada, glow animado e borda inspirada no Plasma Orbit."""
    ...
    @staticmethod
    def _polygon(points):
        left_top, left_bottom, right_top, right_bottom = points
        return [left_top, right_top, right_bottom, left_bottom]

    def draw(self, frame, points, filter_key: str, tick: float):
        cv2, np = self.cv2, self.np
        polygon = np.asarray(self._polygon(points), dtype=np.int32)
        h, w = frame.shape[:2]
        ...
        local_poly = polygon - np.array([x, y], dtype=np.int32)
        ...
        blended = filtered.astype(np.float32) * soft[..., None] + roi.astype(np.float32) * (1.0 - soft[..., None])
...
class HandPortalTracker:
    """Duas mãos, escala adaptativa, handedness e suavização temporal."""
    ...
    @staticmethod
    def _anchors(hand_landmarks, width, height):
        lm = hand_landmarks.landmark
        return (
            (lm[INDEX_TIP].x * width, lm[INDEX_TIP].y * height),
            (lm[THUMB_TIP].x * width, lm[THUMB_TIP].y * height),
        )
    ...
    def detect(self, frame, scale=1.0):
        ...
        processing = frame if scale >= 0.995 else cv2.resize(frame, None, fx=scale, fy=scale, interpolation=cv2.INTER_AREA)
        ...
        raw = [li, lt, ri, rt]
        if (li[0] + lt[0]) > (ri[0] + rt[0]):
            raw = [ri, rt, li, lt]
        smooth = self.smoother.update(raw)
...
class StarVisionPortalApp:
    ...
    def _open_camera(self):
        ...
        if not cap.isOpened():
            raise RuntimeError(f"Não consegui abrir a câmera {self.camera_index}. Verifique permissões e índice da webcam.")
    ...
    def _draw_hud(self, frame, tracking_ok):
        ...
        lines = [
            ...
            "GESTO: aproxime as maos para avancar  |  A/D filtro  |  S captura  |  H HUD  |  F tela cheia  |  Q sair",
        ]
        overlay = frame.copy()
        ...
        if time.monotonic() < self._notice_until:
            text = self._last_notice
            (tw, _), _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.68, 2)
            cv2.putText(frame, text, (max(20, (w - tw) // 2), h - 38), cv2.FONT_HERSHEY_SIMPLEX, 0.68, (255, 255, 255), 2, cv2.LINE_AA)
        return frame
    ...
    def _capture(self, frame):
        ...
        path = CAPTURE_DIR / f"star-vision-{stamp}-{self.state.captures:02d}.png"
        ok = self.cv2.imwrite(str(path), frame)
        self._notice(f"CAPTURA: {path.name}" if ok else "Falha ao salvar captura")
    ...
    def _handle_key(self, key, frame):
        ...
        if key in (ord("d"), ord("D")):
            ...
            cv2.setWindowProperty(self.WINDOW, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN if self.state.fullscreen else cv2.WINDOW_NORMAL)
        return True
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                inst_fps = 1.0 / dt
                self.fps = inst_fps if self.fps <= 0 else self.fps * 0.88 + inst_fps * 0.12
