...
_DEFAULT_ECOSYSTEM = {
    ...
    "labels": {
        ...
        "stop_and_send": "ENVIAR ÁUDIO", "camera": "MOSTRAR À STAR",
        "sensors": "SENSORES", "measure": "MEDIR",
    },
    "features": {
        "text": True, "voice_input": True, "spoken_reply": True, "camera_transport": True,
        "vision_analysis": True, "remote_pc_actions": False,
        "sensor_transport": True, "location_transport": True, "health_transport": True,
        "motion_transport": True, "rotary_input": True, "measure_provider": "hardware-if-available",
    },
    "profiles": {
...
class DeviceRuntime:
    ...
    def __init__(self, manifest_path: Path):
        ...
        self._lock = threading.Lock(); self._source_digest = None
        self._ecosystem = deepcopy(_DEFAULT_ECOSYSTEM); self._star_name = "STAR"; self._star_version = "unknown"
        self._protocol = 1; self._revision = "bootstrap"; self._reload(force=True)
    ...
    def _reload(self, force: bool = False) -> None:
        ...
        star_name = str(manifest.get("name") or "STAR"); star_version = str(manifest.get("version") or "unknown")
        gateway = manifest.get("device_gateway"); gateway = gateway if isinstance(gateway, dict) else {}
        try: protocol = int(gateway.get("protocol", 1))
        except (TypeError, ValueError): protocol = 1
        canonical = json.dumps({"name": star_name, "version": star_version, "protocol": protocol, "ecosystem": ecosystem},
    ...
    def profile_for(self, device_record=None):
        self._reload(); record = device_record if isinstance(device_record, dict) else {}
        ...
        if form_factor not in profiles:
            name = str(record.get("name") or "").lower(); form_factor = "watch" if "watch" in name or "relógio" in name else "phone"
        profile = profiles.get(form_factor) or profiles.get("phone") or {}
        return {
            "revision": revision, "schema": int(ecosystem.get("schema", 1)), "protocol": protocol,
            "sync_interval_seconds": max(10, int(ecosystem.get("sync_interval_seconds", 30))),
            "star": {"name": star_name, "version": star_version}, "form_factor": form_factor,
