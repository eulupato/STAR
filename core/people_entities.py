...
Perfis declarados e evidências perceptivas podem ser associados à entidade sem
armazenar automaticamente biometria bruta. Evidência de identidade continua sendo
evidência, não autenticação nem permissão.
"""
...
AXES = (
    ("source", tuple(str(i) for i in range(10))),
    ("time", tuple(str(i) for i in range(10))),
    ("context", tuple(str(i) for i in range(10))),
    ("confidence", tuple(str(i) for i in range(10))),
    ("relation", tuple(str(i) for i in range(10))),
    ("permission", tuple(str(i) for i in range(10))),
)
...
class PeopleEntities:
    ...
    def _activate(self, person: dict) -> dict:
        ...
        callback = self.on_active_person
        if callable(callback):
            try:
                callback(deepcopy(person))
    ...
    def _safe_profile(profile: dict | None) -> dict:
        ...
        for key, value in list(dict(profile or {}).items())[: PeopleEntities.MAX_PROFILE_FIELDS]:
            ...
            if isinstance(value, (str, int, float, bool)) or value is None:
                output[name] = value if not isinstance(value, str) else _clean(value)[:500]
    ...
    def create_person(
        ...
        *,
        ...
        stable = _norm(name + "|" + source + "|" + reference)
        ...
        mem = self.memory.remember(
            ...
            metadata={
                ...
                "recognition_not_authentication": True,
                "authentication_status": "not_authenticated",
                "sensitive_attribute_inference": False,
            },
        )
    ...
    def find_people(self, name_or_alias: str, *, limit: int = 8) -> list[dict]:
        ...
        recalled = self.memory.recall(
            query,
            kinds=("people",),
            limit=min(max(int(limit) * 4, 8), 64),
        ...
        for item in recalled:
            ...
            if wanted not in labels and wanted not in _norm(item.get("content")):
                continue
            entry = found.setdefault(person_id, {
                "person_id": person_id,
                "name": name or query,
                "aliases": aliases,
                "memory_ids": [],
    ...
    def upsert_declared_person(
        ...
        *,
        ...
        """Reutiliza somente uma correspondência nominal inequívoca; não faz face-merge."""
    ...
    def remember_profile(
        ...
        *,
        ...
        mem = self.memory.remember(
            "people",
            f"Perfil {'declarado' if declared else 'observado'}: {compact}",
            source=source,
            reference=reference,
            entities=[person_id],
            ...
            metadata={
                "person_id": person_id,
                "profile": safe,
                "declared_profile": bool(declared),
                "sensitive_attribute_inference": False,
                "recognition_not_authentication": True,
            },
        )
    ...
    def attach_identity_evidence(
        ...
        *,
        ...
        """Associa referência/evidência; nunca autentica e não persiste biometria bruta."""
        ...
        mem = self.memory.remember(
            "people",
            f"Evidência de identidade ({modality}): {_clean(descriptor) or reference}",
            source=source,
            reference=reference,
            entities=[person_id],
            importance=0.65,
    ...
    def ingest_profile(
        ...
        *,
        ...
        source: str,
        reference: str,
        identity_evidence: Iterable[dict] = (),
    ) -> dict:
        person = self.upsert_declared_person(
            name,
            aliases=aliases,
            source=source,
            reference=reference,
    ...
    def remember_interaction(
        ...
        *,
        ...
        mem = self.memory.remember(
            ...
            context=context,
            importance=importance,
            metadata={"person_id": person_id, "interaction": True},
        )
    ...
    def recognition_hypothesis(self, candidates: Iterable[dict], *, signals: Iterable[dict] = ()) -> dict:
        ranked = sorted(
            (deepcopy(x) for x in candidates if isinstance(x, dict)),
            key=lambda x: _clamp(x.get("confidence"), 0),
            reverse=True,
        )[:8]
        return {
            "status": "hypothesis",
            "candidates": ranked,
            "signals": deepcopy(list(signals or ()))[:16],
            "authenticated": False,
            "grants_permission": False,
    ...
    def authenticate(self, person_id: str, *, verifier=None, challenge: Any = None) -> dict:
        if verifier is None:
            return {
                "person_id": person_id,
                "authenticated": False,
                "reason": "authentication_verifier_unavailable",
        ...
        try:
            ...
            return {
                "person_id": person_id,
                "authenticated": False,
                "reason": "authentication_failed_closed",
                "recognition_used_as_authentication": False,
            }
    ...
    def person_context(self, person_id: str, *, limit: int = 16) -> dict:
        ...
        memories = [
            ...
            for item in self.memory.recall(
                person_id,
                kinds=("people", "social", "episodic", "conversation"),
                limit=bounded,
            )
            if person_id in str(item.get("metadata") or {}) or person_id in _clean(item.get("content"))
        ]
        return {
            "person_id": person_id,
            "memories": memories[:bounded],
            "relations": self.graph.neighbors(person_id, limit=bounded),
            "bounded": True,
        }
    ...
    def stats(self) -> dict:
        return {
            ...
            "namespace": self.knowledge.store.get_namespace("B26"),
            "addressable_contents": ADDRESSABLE_CONTENTS,
            "shared_graph": True,
            "shared_memory": True,
            "recognition_is_authentication": False,
            "profile_ingestion": True,
            "identity_evidence_without_authentication": True,
            "raw_biometric_storage_by_default": False,
        }
    ...
    def handle(self, text: str) -> str | None:
        ...
        if low in {"status bloco 26", "status pessoas", "status entidades pessoais"}:
            return (
                f"👥 BLOCO 26 — PESSOAS: {ADDRESSABLE_CONTENTS} relações/conhecimentos endereçáveis | "
                "memória=B13 | reconhecimento ≠ autenticação."
            )

        # A declaração é persistida aqui, mas retornamos None para preservar a
        # resposta estável do StarCore e o comportamento antigo de sessão.
        match = re.search(r"\bmeu nome (?:e|é)\s+([A-Za-zÀ-ÿ][A-Za-zÀ-ÿ' -]{0,40})", raw, re.I)
        if match:
            name = _clean(match.group(1)).split()[0]
            self.upsert_declared_person(
                name,
