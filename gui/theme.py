"""Tema visual STAR OS local, otimizado para Tkinter/Windows."""
...
def round_rect(canvas: tk.Canvas, x1, y1, x2, y2, radius=18, **kwargs):
    """Desenha retângulo arredondado sem dependências externas."""
...
def draw_starfield(canvas: tk.Canvas, width: int, height: int, *, seed: int = 44, clouds: bool = False):
    """Desenha o fundo espacial de forma leve e determinística.

    A versão anterior simulava gradiente com vários ovais gigantes + stipple.
    Em Tk/Windows isso podia consumir muito tempo de renderização durante
    Configure/hover. Aqui mantemos o mesmo clima visual com poucos elementos.
    """
    ...
    # Vignette leve, sem stipple/alpha caro.
    canvas.create_oval(
        width * .15, height * .03, width * .85, height * .92,
        fill="#0a1120", outline="", tags="star_bg"
    )
    canvas.create_oval(
        width * .28, height * .12, width * .72, height * .78,
        fill="#0c1423", outline="", tags="star_bg"
    )

    count = max(58, min(150, int((width * height) / 15000)))
    for _ in range(count):
        ...
        if size == 3 and rng.random() < .28:
            canvas.create_line(x - 3, y + 1, x + 6, y + 1, fill="#667288", tags="star_bg")
            canvas.create_line(x + 1, y - 3, x + 1, y + 6, fill="#667288", tags="star_bg")
