...
class StarApp:
    ...
    def toggle_microphone(self):
        if not self.voice.stt_configured:
            ...
            self._append_system("🎤 Reconhecimento local ainda não instalado. Execute INSTALAR_VOZ.bat.")
            return
        if not self.recorder.available:
            self._activate_conversation()
            self._append_system("🎤 Não consegui acessar o microfone. Verifique as dependências de áudio da STAR.")
            return
        if not self.recording:
            try:
                ...
                self._activate_conversation()
                self._append_system(f"🎤 Não consegui abrir o microfone: {exc}")
            return
        self.recording = False
            ...
            while True:
                ...
                if kind == "transcript":
                    ...
                    if hasattr(self, "voice_test_label"):
                        self.voice_test_label.config(text=message, fg=self.green if ok else self.red)
    ...
    def _set_status(self, text, color):
        if hasattr(self, "status_label"):
            self.status_label.config(text=f"● V{VERSION} • {text}", fg=color)
    ...
    def show_settings(self):
        ...
        self.online_btn = self._button(row, "🟢 ONLINE", lambda: self._set_mode(True))
        self.online_btn.pack(side="left", padx=(0, 10))
        self.offline_btn = self._button(row, "🔴 OFFLINE", lambda: self._set_mode(False))
        self.offline_btn.pack(side="left")
        self._refresh_mode_buttons()
        tk.Label(mode, text="ONLINE é reservado para recursos que realmente usam internet. A voz da STAR é local.", fg=self.muted, bg=self.panel, wraplength=760, justify="left").pack(anchor="w")
        ...
        tk.Label(voicebox, text="Entrada: faster-whisper local • Saída: Chatterbox local • Reprodução: sounddevice", fg=self.text, bg=self.panel).pack(anchor="w", pady=(8, 2))
        tk.Label(voicebox, text="O Chatterbox mantém o modelo em um worker separado para não recarregar o modelo a cada frase.", fg=self.muted, bg=self.panel, wraplength=760, justify="left").pack(anchor="w")
        self._button(voicebox, "TESTAR VOZ DA STAR", self._test_voice).pack(anchor="w", pady=(12, 4))
        self.voice_test_label = tk.Label(voicebox, text="Pronto para testar.", fg=self.muted, bg=self.panel, wraplength=760, justify="left")
        ...
        for name, value in (
            ...
            ("Chatterbox", "PRONTO" if self.voice.configured else "INSTALAÇÃO PENDENTE"),
            ("Reconhecimento local", "PRONTO" if self.voice.stt_configured else "INSTALAÇÃO PENDENTE"),
        ):
    ...
    def _refresh_mode_buttons(self):
        if not hasattr(self, "online_btn"):
            return
        self.online_btn.config(bg="#1f5a3a" if self.online_mode else "#24313f")
        self.offline_btn.config(bg="#24313f" if self.online_mode else "#5a2630")
    ...
    def show_islands(self):
        ...
        for index, (key, island) in enumerate(get_islands().items()):
            card = tk.Frame(inner, bg=self.panel, padx=16, pady=14)
            card.grid(row=index // 3, column=index % 3, sticky="nsew", padx=6, pady=6)
            tk.Label(card, text=f"{island['icon']} {island['name']}", fg=self.star, bg=self.panel, font=("Segoe UI", 14, "bold")).pack(anchor="w")
            tk.Label(card, text=island["description"], fg=self.text, bg=self.panel, wraplength=270, justify="left").pack(anchor="w", pady=7)
            if key.lower() == "casa":
                self._button(card, "ENTRAR NA CASA", self.show_house, small=True).pack(anchor="w", pady=5)
            else:
                installed = island.get("status") == "installed"
                tk.Label(card, text="🟢 DISPONÍVEL" if installed else "🔒 EM PREPARAÇÃO", fg=self.green if installed else self.gold, bg=self.panel, font=("Segoe UI", 8, "bold")).pack(anchor="w")
    ...
    def show_house(self):
        ...
        grid.pack(fill="x")
        for index, (title, description, command) in enumerate((("🍳 COZINHA", "Receitas, culinária e experimentação gastronômica.", None), ("👕 CLOSET", "Skins, roupas, aparência e personalização visual.", self.show_closet))):
            card = tk.Frame(grid, bg=self.panel, padx=22, pady=20, width=360, height=180)
            card.grid(row=0, column=index, padx=(0, 14), sticky="nsew")
            card.grid_propagate(False)
            tk.Label(card, text=title, fg=self.star, bg=self.panel, font=("Segoe UI", 16, "bold")).pack(anchor="w")
            tk.Label(card, text=description, fg=self.text, bg=self.panel, wraplength=290, justify="left").pack(anchor="w", pady=12)
            if command:
                self._button(card, "ABRIR CLOSET", command).pack(anchor="w")
    ...
    def show_closet(self):
        ...
        top.pack(fill="x", padx=45, pady=(25, 0))
        ...
        if not self.closet_files:
            tk.Label(body, text="Nenhuma skin encontrada na pasta SKINS.", fg=self.red, bg=self.bg).pack(pady=80)
            return
        try:
            self.closet_index = [p.name for p in self.closet_files].index(self.selected_skin)
        except ValueError:
            self.closet_index = 0
        area = tk.Frame(body, bg=self.bg)
        area.pack(fill="both", expand=True)
        self._button(area, "◀", lambda: self._change_closet_skin(-1)).place(relx=0.18, rely=0.5, anchor="center")
        self._button(area, "▶", lambda: self._change_closet_skin(1)).place(relx=0.82, rely=0.5, anchor="center")
        card = tk.Frame(area, bg="#172231", padx=18, pady=16)
        card.place(relx=0.5, rely=0.47, anchor="center", width=430, height=455)
        self.closet_image = tk.Label(card, bg="#172231")
        self.closet_image.pack(expand=True, fill="both")
        self.closet_name = tk.Label(card, fg=self.text, bg="#172231", font=("Segoe UI", 14, "bold"))
        self.closet_name.pack(pady=(8, 4))
        self.closet_state = tk.Label(card, fg=self.green, bg="#172231", font=("Segoe UI", 9, "bold"))
    ...
    def restore_normal_size(self, _event=None):
        if self.is_maximized:
            ...
            self.window.geometry(f"{max(900, self.normal_size[0])}x{max(600, self.normal_size[1])}")
            self.is_maximized = False
